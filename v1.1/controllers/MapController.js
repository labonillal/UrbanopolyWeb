...
exports.getMapData = function (req, res){
	...
	var requestUrl = str.format(Utility.getServerAddress(), servicePlayArea, req.user.id, 45.480265, 9.223666, gameConf.maxDistance);
    	...
    	if (!error && response.statusCode === 200) {
			...
			for(var i = 0; i < venuesJson.length; i++){
				...
				if(venuesJson[i]["state"] == 'MINE'){
					...
					if(venuesJson[i]["value"] < gameConf.expensiveVenueThreshold){
			...
			GeoJSON.parse(venuesCollection, {Point: ['lat', 'lon'],
											 extra: {
												"title": "Cheap Venue"
												}
											}, function(geojson){

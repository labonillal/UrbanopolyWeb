...
function createMap(lat, lon) {
	//console.log('Location Coordinates: ' + lon +' , ' + lat);
		...
		myMarkersLayer.on('click',function(e) {
    		...
    		//console.log('VENUES VISITED: ', venuesVisited);
    		//Messages setup
			$("#infoMessage").hide();
			$("#successMessage").hide();
			$("#errorMessage").hide();
    		...
    		if(venuesVisited.indexOf(e.layer.feature.properties["id"]) != -1){
	    		...
	    		//Occupied Venue
					...
					if(typeof e.layer.feature.properties.quiz != 'undefined'){
						...
						for(var i in questions){
							...
							if(i == 0){
								//Nav tab active
								$('<li class="active"><a data-toggle="tab"> <strong>Question '+ currentStep +'</strong> </a> </li>').appendTo('#quizTabsNav');
								//Tab pane active
								...
								//Nav tab
								$('<li> <a data-toggle="tab"> <strong>Question '+ currentStep +'</strong> </a></li>').appendTo('#quizTabsNav');
								//Tab pane
	    			...
	    			if(typeof e.layer.feature.properties.advertise != 'undefined'){
	    				...
	    				for(var i in featureTypes){
	    					//console.log('For loop iteration '+ i +' starts...');
	    					var featureType = featureTypes[i];
	    					//console.log('featureType['+i+'] : ', featureType);
							currentStep++;
							nextStep++;
							//Nav tab
							$('<li id= "adv'+ currentStep +'"> <a data-toggle="tab"> <strong>Step '+ currentStep +'</strong> </a></li>').appendTo('#advertiseTabsNav');
							//Tab pane
						...
						var content =	'<div class="tab-pane" id="advStep'+ currentStep +'">';
							...
							content +=					'<div class="alert alert-info">';
							content +=						'<strong>'+ currentStep +'.</strong> Upload a picture.';
							content +=					'</div>';
							content +=				'</div>';
							content +=				'<div class="row" style="text-align:center">';
							content +=					'<form enctype="multipart/form-data">';
							content +=					    '<input id="photo" name="file" type="file" />';
							...
							content +=					'<div class="panel-heading">'
							content +=						'<div class="row" style="text-align:center">';
							content +=							'<output id="thumbnail">';
							content +=								'<span>';
							content +=									'<img class="thumb" src="/images/take_photo.png" title="default_photo">';
							content +=								'</span>';
							content +=							'</output>';
							content +=						'</div>';
							content +=					'</div>';
							content +=					'<div class="panel-body">'
							content +=						'<div id="posterDetails" class="col-md-12">';
							content +=						'</div>';
							content +=					'</div>';
							content +=				'</div>';
							content +=			'</div>';
	    		...
	    		//My Venue
	    			...
	    			// Retrieves visit judge if its available
	    			if(typeof e.layer.feature.properties.judge != 'undefined'){
	    				visitJudge = e.layer.feature.properties.judge;
	    			}
...
function requestUpdatedMap(lat, lon) {
	//Retrieve the venues visited by the player
	loadAlreadyVisitedVenues();
...
function loadAlreadyVisitedVenues() {
	//console.log('RetrieveAlreadyVisitedVenues start...');
	var url = '/RetrieveAlreadyVisitedVenues';
	//console.log('REQUEST URL: ', url);
	$.ajax(
		{
            type: 'GET',
            url: '/RetrieveAlreadyVisitedVenues',
            success: function (result) {
            	if(typeof result != "undefined"){
	            	venuesVisited = result;
					//console.log('loadAlreadyVisitedVenues finish...')
				}
            },
            error: function (req, status, error) {
				//console.log('ERROR: ' + error);
				console.log('Unable to get already visited venues for the player');
            }
        });
};
...
$("#nextAdvStp1").click(function() {
	var posterDetails = 	'<h4>'+ $('#advertiseInputVenueName').val() +'</h4>';
		posterDetails +=	'<h5>'+ $("#advertiseParentSelect option:selected").text() + ' (' + $("#advertiseChildSelect option:selected").text() +')</h5>';
...
$('#advertiseTabsContent').on( "click", "a.btn", function(event){
	...
	switch(event.target.id) {
		case 'okAdvStp2':
			...
			//add details to the poster
			var posterDetails = '<h5>' + visitAdvertise.featureTypes[0].name + ' : ' + $('#advStep2Input').val() +'</h5>';
			$(posterDetails).appendTo('#posterDetails');
			break;
		case 'idkAdvStp2':
			if(visitAdvertise.featureTypes[0].featureRange.type == 'open'){
		...
		case 'okAdvStp3':
			...
			//add details to the poster
			var posterDetails = '<h5>' + visitAdvertise.featureTypes[1].name + ' : ' + $('#advStep3Input').val() +'</h5>';
			$(posterDetails).appendTo('#posterDetails');
			break;
		case 'idkAdvStp3':
			if(visitAdvertise.featureTypes[1].featureRange.type == 'open'){
		...
		case 'okAdvStp4':
			...
			//add details to the poster
			var posterDetails = '<h5>' + visitAdvertise.featureTypes[2].name + ' : ' + $('#advStep4Input').val() +'</h5>';
			$(posterDetails).appendTo('#posterDetails');
			break;
		case 'idkAdvStp4':
			if(visitAdvertise.featureTypes[2].featureRange.type == 'open'){
		...
		case 'uploadBtn':
			...
			break;
		case 'finishAdvStp5':
			visitAdvertise.featureValues[3] = 'undefined';
			featureRanges[3] = 'undefined';
			visitAdvertise.skips[3] = 'true';
			advertise(visitAdvertise.featureValues, visitAdvertise.skips, featureRanges);
			//console.log('VISIT ADVERTISE :', visitAdvertise);
			//console.log('FEATURE VALUES :', visitAdvertise.featureValues);
			//console.log('SKIPS :', visitAdvertise.skips);
...
function uploadPhoto(){
	//console.log('uploadPhoto starts...');
    ...
    if (!input) {
		...
		//Setup the request with the venue data
		var venue = JSON.parse(selectedVenue);
	...
	//console.log('uploadPhoto finish...');
}
...
$("#advertiseTabsContent").on('change',':file' , function(){
	$("#errors").empty();
    ...
    if(!type.match('image.*')){
    	...
    	document.getElementById("errors").innerHTML = '<h5 style="color:red">File is not an image</h5>';
    }else{
	    ...
	    reader.onload = (function(theFile) {
			return function(e) {
				// Render thumbnail instead of the default one.
				$("#thumbnail").empty();

...
//Method to perform the JUDGE action
exports.judge = function(venueJudge, photoRank, player, venueCategory, venueName, featureValues, offensive, callback){
  console.log('GameController.judge starts...');
  //console.log('VENUE BEFORE PARSE: ', venueJudge);
  var venue = Venue.parse(venueJudge);
  //console.log('PLAYER: %j', player);
  //console.log('VENUE AFTER PARSE: %j', venue);
  //console.log('PHOTORANK: %j', photoRank);
  //console.log('CATEGORY: %j', venueCategory);
  //console.log('FEATURE_VALUES: %j', featureValues);
  //console.log('OFFENSIVE: %j', offensive);
  // Not undefined validation
  if(typeof venue == 'undefined' && typeof player == 'undefined'){
    console.log('ERROR: Undefined');
    var error = {"code": 0 };
  ...
  // Set player cash
  player.cash += parseInt(gameConf.moneyForJudge);
  //console.log('player.cash: ', player.cash);

  // Get visits of judge type
  var visitJudge = visitController.getVisitJudge(venue);
  console.log('visitJudge: ', visitJudge);

  var visit;
  if(typeof visitJudge != 'undefined'){
    visit = VisitJudge.parse(visitJudge);
    console.log('visit after parse: ', visit);
  }

  if(typeof visit != 'undefined'){
    visit.date = Utility.getCurrentDate();
    visit.deltaMoney = parseInt(gameConf.moneyForJudge);
    visit.venueCategory = venueCategory;
    visit.venueName = venueName;
    visit.featureValues = featureValues;
    visit.photoRank = photoRank;
    visit.offensive = offensive;
    console.log('visit after set values: ', visit);

    // Add it to the array of visits
    visitController.addOutVisit(visit);
    console.log('after addOutVisit!');
